...
- A renter can withdraw her own not-yet-confirmed `INTERESTED` booking herself (hard-deleted — nothing meaningful to preserve), instead of only being able to wait out the 7-day auto-expiry or ask the owner to do it; cancelling an already-`RENTED` booking still requires the owner, since that's a real confirmed rental
...
- "הבקשות שלי" (my requests) page listing everything the current user has booked as a renter, with the same chat thread as the owner sees, and a self-service "ביטול העניין" (withdraw interest) action on any still-pending request
...
- Revocable sessions despite stateless JWTs: every user carries a `tokenVersion`, embedded in each issued token and checked on every authenticated request — bumping it (from account settings' "התנתקות מכל המכשירים", or automatically on every password change/reset) instantly invalidates every token issued before that moment, everywhere, without needing a separate session store. A password change issues a fresh token for the device that just changed it, so that one stays logged in while every other session is cut
